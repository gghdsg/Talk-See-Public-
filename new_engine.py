...
start = time.time()
Marine_data = np.load("/mnt/disk6new/wzq/experiment/VBS/VbsBackend/competition/npdata/marine_data_norm.npy").astype(
    np.float32)
...
V3C_data = np.load("/mnt/disk6new/wzq/experiment/VBS/VbsBackend/competition/npdata/V3C_data_norm.npy").astype(
    np.float32)
...
LHE_data = np.load("/mnt/disk6new/wzq/experiment/VBS/VbsBackend/competition/npdata/LHE_data_norm.npy").astype(
    np.float32)
...
def save_index(calc_type="cos"):
    global faiss_index, faiss_index_Marine
    global faiss_index_V3C, faiss_index_LHE
    faiss.write_index(faiss_index_Marine, "Marine.index")
    faiss.write_index(faiss_index_V3C, "V3C.index")
    faiss.write_index(faiss_index_LHE, "LHE.index")

def read_index():
    global faiss_index, faiss_index_Marine
    global faiss_index_V3C, faiss_index_LHE
    faiss_index_Marine = faiss.read_index("Marine.index")####
    faiss_index_LHE = faiss.read_index("LHE.index")
    faiss_index_V3C = faiss.read_index("V3C.index")
index_type = 'L2'
# create_index(index_type)
# save_index()
start_read=time.time()
read_index()
print("时间：{:.3f}s".format(time.time() - start_read))
# 根据文件路径读取图片的方法
...
def changeurl(text):  # 换成绝对路径
    return '/mnt/disk6new/fsw/VBS/' + text[33:]
...
def searchListByText(text, datatype, refresh):
...
def send_gpt(modeltype, channel=0):
    ...
    if modeltype == 'gpt':
        ...
        answer = ""  # llama2_api.llama2chat(text)
    print(answer)
...
def findNextFolder(sample):
    ...
    if os.path.isfile(sample_static):
        ...
        numstr = 0
        print(segments)
        if segments[0] == 'hkust-vgd.ust.hk' or segments[0] == 'VBSLHE_keyframes1':
